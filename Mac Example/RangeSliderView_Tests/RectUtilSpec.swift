  ...
  override func spec() {
    ...
    describe("RectUtil") {
      ...
      it("can calculate the center of a view given x location") {
        let rect = RectUtil.updateRectHorizontalCenter(30)(CGRect(x: 0, y: 0, width: 20, height: 20))
        expect(rect) == CGRect(x: 20, y: 0, width: 20, height: 20)
      }
      
      it("can center a rect vertically") {
        let rect = RectUtil.centerRectVertically(50)(CGRect(x: 0, y: 0, width: 20, height: 20))
      ...
      it("can constraint a rect within bounds") {
        ...
        expect(function(CGRect(x: 0, y: 0, width: 20, height: 20))) == CGRect(x: 20, y: 0, width: 20, height: 20)
        expect(function(CGRect(x: 30, y: 0, width: 20, height: 20))) == CGRect(x: 30, y: 0, width: 20, height: 20)
        expect(function(CGRect(x: 130, y: 0, width: 20, height: 20))) == CGRect(x: 80, y: 0, width: 20, height: 20)
      }
      
      it("can set a rect height") {
        let newRect = CGRect(x: 0, y: 0, width: 20, height: 20) |> RectUtil.setRectHeight(30)
      ...
      it("can apply bound range on a rect") {
        let newRect = CGRect(x: 0, y: 0, width: 20, height: 20)
          |> RectUtil.applyBoundRange(BoundRange(start: 10, end: 130))
